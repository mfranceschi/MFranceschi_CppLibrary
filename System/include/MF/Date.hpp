...
//------------------------------------------------------ Other definitions
    ...
    constexpr bool Date::IsLeapYear(int year) noexcept { return (!(year & 0b11) && (year % 100)) || !(year % 400); }
    ...
    constexpr bool Date::ValidateSeconds(int d) noexcept { return d >= 0 && d <= 60; }

    constexpr bool Date::ValidateMinutes(int d) noexcept { return d >= 0 && d < 60; }

    constexpr bool Date::ValidateHours(int d) noexcept { return d >= 0 && d < 24; }

    constexpr bool Date::ValidateDays(int d, int m) noexcept {
        return d >= 1 && d <= (Date::ValidateMonths(m) ? d <=
                                                         Date::DaysInMonth(
                                                                 m)
                                                       : 31);
    }

    constexpr bool Date::ValidateMonths(int d) noexcept { return d >= JANUARY && d <= DECEMBER; }

    inline time_t Date::Now_Timestamp() { return std::time(nullptr); }
    ...
    inline bool Date::operator<(const Date &b) const { return Compare(b) == Toolbox::INFERIOR; }

    inline bool Date::operator>(const Date &b) const { return Compare(b) == Toolbox::SUPERIOR; }
