...
const File::SFilename_t FILENAME_MIDDLE_SIZE =
    File::MakeFilename(false, false, 2, TEST_FILES_DIR_PREFIX, MIDDLE_SIZE_RAW);
const File::SFilename_t FILENAME_NOT_EXISTING =
    File::MakeFilename(false, false, 2, TEST_FILES_DIR_PREFIX, NOT_EXISTING_RAW);
const File::SFilename_t FILENAME_SMALL_UTF16LE =
    File::MakeFilename(false, false, 2, TEST_FILES_DIR_PREFIX, SMALL_UTF16LE_RAW);
const File::SFilename_t FILENAME_TEMP =
    File::MakeFilename(false, false, 2, TEST_FILES_DIR_PREFIX, TEMP_RAW);
...
class TestFile : public ::testing::Test {
   protected:
    ...
    void CheckSize() const {
        File::Filesize_t size = File::GetFileSize(fid.name.c_str());
        if (fid.size) ASSERT_NE(size, 0);
        EXPECT_EQ(size, fid.size);
    }

    void CheckExists() const {
        bool result = File::IsFile(fid.name.c_str());
        if (fid.exists)
            EXPECT_TRUE(result);
        else
            EXPECT_FALSE(result);
    ...
    void CheckEncoding() const {
        EXPECT_EQ(File::GetFileEncoding(fid.name.c_str()), fid.encoding);
    }

    void CheckOpen() const {
        std::ifstream ifs;
        bool was_opened = File::OpenFile(ifs, fid.name.c_str());
        if (fid.exists) {
    ...
    void CheckIsEmpty() const {
        if (!fid.exists) {
            EXPECT_FALSE(File::IsFileReadable(fid.name.c_str(), 1));
            EXPECT_FALSE(File::IsFileReadable(fid.name.c_str()));
            EXPECT_FALSE(File::IsFileReadable(fid.name.c_str(), -1));
        } else {
            ...
            EXPECT_TRUE(File::IsFileReadable(fid.name.c_str(), 0));
            EXPECT_TRUE(File::IsFileReadable(fid.name.c_str(), -1));

            // Read a few times.
            for (unsigned int i = 1; i < chars_max; ++i) {
                EXPECT_TRUE(File::IsFileReadable(fid.name.c_str(), i))
                    << "Fail of \"Read a few times\" with index " << i;
            }
            if (fid.size < 3 /* Value of default param. */)
                EXPECT_FALSE(File::IsFileReadable(fid.name.c_str()));
            else
                EXPECT_TRUE(File::IsFileReadable(fid.name.c_str()));
        }
    }

    void SetUp() override {
        if (fid.exists) {
            ASSERT_TRUE(File::IsFile(fid.name.c_str())) << File::GetCWD() << std::endl << fid.name;
        } else {
            if (File::IsFile(fid.name.c_str())) {
    ...
    void TearDown() override {
        if (fid.exists)
            ASSERT_TRUE(File::IsFile(fid.name.c_str()));
        else
            ASSERT_FALSE(File::IsFile(fid.name.c_str()));
    }
};
...
TEST(IsDir, IsAFile) {
    ...
    ASSERT_TRUE(File::IsFile(fname.c_str()));
...
TEST(IsDir, NewFolder) {
    ...
    ASSERT_TRUE(File::CreateDirectory(filename.c_str()));
...
TEST(Read, ManyReads) {
    ...
    for (long i = 0; i < iterations; ++i) {
        auto fileData = File::ReadWholeFile(file.c_str());
...
TEST(Read, StringRead) {
    ...
    EXPECT_TRUE(File::ReadWholeFileToString(file_used.name.c_str(), callResult))
...
TEST(ListFilesInDirectory, UsualTest) {
    std::vector<File::SFilename_t> ret =
        File::ListFilesInDirectory(TEST_FILES_DIR_PREFIX FILE_SEPARATOR);
