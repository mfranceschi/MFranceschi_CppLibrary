...
namespace detail {
    ...
    public:
        ...
        explicit constexpr ArrayIterator(Array<T,N>& ar, std::size_t index) :
            _ar(ar), _index(index)
        { }
        ...
        // ----- Operators
        constexpr reference operator* () const { return _ar[_index]; }
        constexpr pointer operator-> () const { return &_ar[_index]; }
        constexpr reference operator[] (difference_type diff) const { return _ar[_index + diff]; } // TODO ensure it is correct
        ...
        constexpr ArrayIterator operator+(difference_type n) const {
            return ArrayIterator<T,N>(_ar, _index + n);
        }
